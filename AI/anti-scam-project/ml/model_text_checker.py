...
class Trainer:
    ...
    def simpan_model(self, path=None):
        """
        Simpan HANYA weight model (.pt). Tokenizer disimpan terpisah
        karena tokenizer bukan angka weight, tapi kamus kata -> token.
        """
        if path is None:
            path = os.path.join(BASE_DIR, "models", "model_text_checker.pt")
        torch.save(self.model.state_dict(), path)
        print(f"Model tersimpan ke: {path}")


def simpan_tokenizer(tokenizer, path=None):
    if path is None:
        path = os.path.join(BASE_DIR, "models", "tokenizer_text_checker")
...
def load_dataset(csv_path=None):
    if csv_path is None:
        csv_path = os.path.join(BASE_DIR, "data", "dataset_scam_starter.csv")
...
# ============================================
def load_model_untuk_prediksi(model_path=None, tokenizer_path=None):
    if model_path is None:
        model_path = os.path.join(BASE_DIR, "models", "model_text_checker.pt")
    if tokenizer_path is None:
        tokenizer_path = os.path.join(BASE_DIR, "models", "tokenizer_text_checker")
...
if __name__ == "__main__":
    ...
    # batch_size=2 supaya nggak kehabisan RAM saat training di CPU
    # (IndoBERT cukup besar ~440MB, backprop butuh memory ekstra)
    train_loader = DataLoader(train_dataset, batch_size=2, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=2)

    model = ModelTextChecker()
    # Aktifkan gradient checkpointing untuk hemat memory saat training
    ...
    trainer.simpan_model()
    simpan_tokenizer(tokenizer)
