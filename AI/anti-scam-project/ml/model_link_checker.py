...
from ml.domain_utils import ekstrak_domain, normalisasi_unicode, deteksi_karakter_mencurigakan, hitung_kemiripan
...
class Trainer:
    ...
    def simpan_model(self, path=None):
        if path is None:
            path = os.path.join(BASE_DIR, "models", "model_link_checker.pt")
...
def load_dataset(csv_path=None):
    if csv_path is None:
        csv_path = os.path.join(BASE_DIR, "data", "dataset_link_scam.csv")
...
if __name__ == "__main__":
    ...
    train_urls, val_urls, train_labels, val_labels = load_dataset()
